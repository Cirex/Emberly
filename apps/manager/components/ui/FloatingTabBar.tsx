 ...
 * bottom bar), ported from the maintenance app: one near-full-width capsule of
 * equal-width icon-over-label tabs whose selected tab carries a FULL-HEIGHT
 * light-glass lozenge with accent-tinted content, plus a round search bubble
 * beside it on the screens whose lists the search filters. Tapping search
 * springs the capsule down into a round button showing the active tab's icon
 * while the bubble stretches into the full-width field — both surfaces stay
 * mounted so the blur never pops. When a screen has no search, the capsule
 * springs out to take the whole row.
 *
 * Manager cut: only the property map is searchable for now (its unit search
 * lives in lib/stores/map-search.ts). Wave-2 boards that grow a text search
 * add their route + store binding below, exactly as maintenance binds its
 * work-order search.
 */
export function FloatingTabBar({ state, descriptors, navigation }: FloatingTabBarProps) {
  ...
  const searchable = onMap;

  // Glass recipe (mirrors AppCardSurface, inlined because these two surfaces
  ...
  // The field binds the map's unit search on the map route.
  const search = useMapSearch((s) => s.query);
  const setSearch = useMapSearch((s) => s.setQuery);
  const placeholder = "Unit, resident, or street";
