...
3. **Scheduled tasks.** The container idles (`CMD ["sleep", "infinity"]`) and the work is
   ...
   **Commands must call `src/run-*.ts` directly.** Every `sync:*` package script except
   ...
   | --- | --- | --- | --- | --- |
   | `sync-core` | `bun run src/run-units.ts && bun run src/run-unit-info.ts && bun run src/run-available-units.ts && bun run src/run-delinquency.ts` | `*/15 * * * *` | every 15 min | 600 |
   | `sync-work-orders` | `bun run src/run-work-orders.ts && bun run src/run-translate-work-orders.ts` | `*/10 * * * *` | every 10 min | 300 |
   ...
   `sync-core` and `sync-work-orders` meet at :00 and :30. Both take the one ResMan lock
   (`src/shared/run-lock.ts`), so `run-work-orders` waits up to 90s for `sync-core`
   (~26s) instead of skipping — a skip there leaves the work-order mirror 20 minutes stale.
